...
namespace Starwars
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // Using lambda and StartsWith to solve assignment 1!
            planets.
                Where( x => x.Name.StartsWith("M")).sendToConsole();

            // Using lambda and Contains to solve assignment 2!
            planets.
                Where(x => x.Name.ToUpper().Contains("Y")).sendToConsole();

            // Using lambda, Length and <> operators to solve assignment 3!
            planets.
                Where(x => x.Name.Length > 9 && x.Name.Length < 15).sendToConsole();

            // Using lambda, specific element index and Endswith to solve assignment 4!
            planets.
                Where(x => x.Name[1] == 'a' && x.Name.EndsWith("e")).sendToConsole();

            // Using lambda, > operator and OrderBy to solve assignment 5!
            planets.
                Where(x => x.RotationPeriod > 40)
                .OrderBy(x => x.RotationPeriod).sendToConsole();

            // Using lambda, <> operators and OrderBy to solve assignment 6!
            planets.
                Where(x => x.RotationPeriod > 10 && x.RotationPeriod < 20)
                .OrderBy(x => x.Name).sendToConsole();

            // Using lambda, > operator, OrderBy and ThenBy to solve assignment 7!
            planets.
                Where(x => x.RotationPeriod > 30)
                .OrderBy(x => x.Name)
                .ThenBy(x => x.RotationPeriod).sendToConsole();

            // Using lambda, <> operators, Contains, OrderBy and ThenBy to solve assignment 8!
            planets.
                Where(x => (x.RotationPeriod < 30 || x.SurfaceWater > 50) && x.Name.ToLower().Contains("ba"))
                .OrderBy(x => x.Name).ThenBy(x => x.SurfaceWater)
                .ThenBy(x => x.RotationPeriod).sendToConsole();

            // Using lambda, > operator and OrderByDescending to solve assignment 9!
            planets.
                Where(x => x.SurfaceWater > 0)
                .OrderByDescending(x => x.SurfaceWater).sendToConsole();

            // Using lambda, >= operator, OrderBy and Math.PI to solve assignment 10!
            planets.
                Where(x => (x.Diameter != 0 && x.Population != 0))
                .OrderBy(x => ((4 * Math.PI * (x.Diameter / 2) * (x.Diameter / 2) / x.Population))).sendToConsole();

            // Using lambda, Except and > operator to solve assignment 11!
            planets.
                Except(planets.Where(x => x.RotationPeriod > 0)).sendToConsole();

            // Using lambda, StartsWith, EndsWith and Contains to solve assignment 12!
            IEnumerable<Planet> assignment12List = planets.Where(x => x.Name.StartsWith("A") || x.Name.EndsWith("s"));
            IEnumerable<Planet> assignment12_V2List = planets.Where(x => x.Terrain != null && x.Terrain.Contains("rainforests"));

            // Using Union to combine multiple data from the lists above!
            List<Planet> ass12 = assignment12List.Union(assignment12_V2List).ToList();

            // Using foreach loop to query execution!
            foreach (Planet assignment12 in ass12)
            {
                Console.WriteLine(assignment12.Name);
            }

            Console.WriteLine("======================");

            // Using lambda and Any to solve assignment 13!
            planets.
                Where(x => x.Terrain != null && x.Terrain.Any(y => y.Contains("desert"))).sendToConsole();

            // Using lambda, Any, OrderBy and ThenBy to solve assignment 14   -  the assignment says find all elements in climate that consists of swamp, but swamp is listed in the terrain column! 
            planets
                .Where(x => x.Terrain != null && x.Terrain.Any(y => y.Contains("swamp")))
                .OrderBy(x => x.RotationPeriod)
                .ThenBy(x => x.Name).sendToConsole();

            // Create a Regex to solve assignment 15  -  \\1 is a back reference to captures all double vocals of aeiou, not just the first one in a word!
            Regex regexOne = new Regex("([aeiou])\\1");

            // Using lambda and IsMatch!
            planets.
                Where(x => regexOne.IsMatch(x.Name)).sendToConsole();

            // Create a Regex to solve assignment 16  -  \\1 is a back reference to captures all double consonants of klnr, not just the first one in a word!
            Regex regexTwo = new Regex("([klnr])\\1");

            // Using lambda, IsMatch and OrderByDescending!
            planets.
                Where(x => regexTwo.IsMatch(x.Name))
                .OrderByDescending(x => x.Name).sendToConsole();
